...
class MaxOnesCountProblem(Problem):
    """
    ...
    Methods:
        __init__(dim: int): Initializes a new instance of the MaxOnesCountProblem class.
        ...
        __str__() -> str: String representation of the MaxOnesCountProblem structure.
        __repr__() -> str: Representation of the MaxOnesCountProblem instance.
        __format__(spec: str) -> str: Formatted MaxOnesCountProblem instance.
    """
    
    def __init__(self, dim:int)->None:
        """
        Create new `MaxOnesCountProblem` instance

        :param int dim: dimension of the problem
        """
        if not isinstance(dim, int):
            raise TypeError('Parameter \'dimension\' for  MaxOnesCountProblem should be \'int\'.')
        if dim <= 0:
            raise ValueError('Parameter \'dimension\' for  MaxOnesCountProblem should be greater than zero.')
        super().__init__(name="MaxOnesCountProblem", is_minimization=False, is_multi_objective=False)
    ...
    def from_input_file(cls, input_file_path:str, input_format:str):
        ...
        """
        dimension:int = MaxOnesCountProblem.__load_from_file__(input_file_path, input_format)
