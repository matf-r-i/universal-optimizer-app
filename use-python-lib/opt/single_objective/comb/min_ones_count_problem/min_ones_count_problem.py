...
class MinOnesCountProblem(Problem):
    
    def __init__(self, dim:int)->None:
        """
        Create new `MinOnesCountProblem` instance

        :param int dim: dimension of the problem
        """
        if not isinstance(dim, int):
            raise TypeError('Parameter \'dimension\' for  MinOnesCountProblem should be \'int\'.')
        if dim <= 0:
            raise ValueError('Parameter \'dimension\' for  MinOnesCountProblem should be greater than zero.')
        super().__init__(name="MinOnesCountProblem", is_minimization=True, is_multi_objective=False)
    ...
    def from_input_file(cls, input_file_path:str, input_format:str):
        ...
        """
        dimension:int = MinOnesCountProblem.__load_from_file__(input_file_path, input_format)
