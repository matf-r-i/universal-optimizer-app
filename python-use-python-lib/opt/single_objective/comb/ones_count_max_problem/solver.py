    ...
    try:
        ...
        if write_to_output_file:
            if parameters['outputFileNameAppendTimeStamp'] is None:
                ...
                should_add_timestamp_to_file_name: bool = bool(
                    parameters['outputFileNameAppendTimeStamp'])
        ...
        if (int(parameters['randomSeed']) > 0):
            ...
            logger.info(
                'RandomSeed is predefined. Predefined seed value:  %d' % r_seed)
            if write_to_output_file:
                output_file.write(
                    '# RandomSeed is predefined. Predefined seed value:  %d\n' % r_seed)
            seed(r_seed)
        else:
            r_seed = randrange(sys.maxsize)  # NOSONAR
            logger.info(
                'RandomSeed is not predefined. Generated seed value:  %d' % r_seed)
            if write_to_output_file:
                output_file.write(
                    "# RandomSeed is not predefined. Generated seed value:  %d\n" % r_seed)
            seed(r_seed)
        ...
        calculation_solution_distance_cache_is_used: bool = parameters[
            'solutionDistanceCalculationCacheIsUsed']
        calculation_solution_distance_cache_max_size: int = parameters[
        ...
        problem = OnesCountMaxProblem.from_input_file(
            input_file_path=input_file_path, input_format=input_format)
        start_time = datetime.now()
        if write_to_output_file:
            output_file.write("# {} started at: {}\n".format(
                parameters['algorithm'], str(start_time)))
            output_file.write(
        ...
        if parameters['algorithm'] == 'variable_neighborhood_search':
            ...
            if solution_type == 'BitArray':
                ...
                raise ValueError(
                    "Invalid solution/representation type is chosen.")
            ...
            solver: OnesCountMaxProblemSolver = OnesCountMaxProblemSolver.from_total_enumeration(
                te_construction_params)
        ...
        logger.debug(
            'Method -{}- search finished.'.format(parameters['algorithm']))
        logger.info('Best solution code: {}'.format(
            bs.string_representation()))
        ...
        logger.info(
            'Execution: {} - {}'.format(solver.opt.execution_started, solver.opt.execution_ended))
