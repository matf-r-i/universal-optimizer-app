...
from uo.algorithm.metaheuristic.electro_magnetism_like_metaheuristic.em_attraction_support_one_point_bit_array import EmAttractionSupportOnePointBitArray
from uo.algorithm.metaheuristic.electro_magnetism_like_metaheuristic.em_mutation_support_one_point_bit_array import EmMutationSupportOnePointBitArray
from uo.algorithm.metaheuristic.electro_magnetism_like_metaheuristic.em_direction_support_one_point_bit_array import EmDirectionSupportOnePointBitArray
...
def main():
    ...
    problem_to_solve: MinSetCoverProblem = MinSetCoverProblem(
        universe_set, subsets)
    solution: MinSetCoverProblemBitArraySolution = MinSetCoverProblemBitArraySolution()
    finish: FinishControl = FinishControl(
        criteria='iterations', iterations_max=100)
    em_attraction_support: EmAttractionSupportOnePointBitArray[str] = EmAttractionSupportOnePointBitArray[str](
    )
    em_mut_support: EmMutationSupportOnePointBitArray[str] = EmMutationSupportOnePointBitArray(
        mutation_probability=0.05)
    em_dir_support: EmDirectionSupportOnePointBitArray[str] = EmDirectionSupportOnePointBitArray[str](
    )
    em_construction_params: EmOptimizerConstructionParameters = EmOptimizerConstructionParameters()
    ...
    optimizer: EmOptimizerGenerational = EmOptimizerGenerational.from_construction_tuple(
        em_construction_params)
    optimizer.optimize()

    # print('Best solution representation: {}'.format(optimizer.best_solution.representation.bin))
    print('Best solution code: {}'.format(
        optimizer.best_solution.string_representation()))
    print('Best solution objective: {}'.format(
        optimizer.best_solution.objective_value))
    print('Best solution fitness: {}'.format(
