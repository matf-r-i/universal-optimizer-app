...
def main():
    ...
    problem_to_solve: MinMultiCutProblem = MinMultiCutProblem(
        graph, source_terminal_pairs)
    solution: MinMultiCutProblemBitArraySolution = MinMultiCutProblemBitArraySolution()
    finish: FinishControl = FinishControl(
        criteria='evaluations', evaluations_max=500)
    num_edges: int = len(problem_to_solve.graph.edges())
    vns_shaking_support: VnsShakingSupportStandardBitArray = VnsShakingSupportStandardBitArray(
        dimension=num_edges)
    vns_ls_support: VnsLocalSearchSupportStandardBestImprovementBitArray = VnsLocalSearchSupportStandardBestImprovementBitArray(
        dimension=num_edges)
    ...
    optimizer.optimize()
    print('Best solution representation: {}'.format(
        optimizer.best_solution.representation.bin))
